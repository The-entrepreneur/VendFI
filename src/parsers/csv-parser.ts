...
function normalizeRow(
  ...
  try {
    ...
    const vendorId = normalizeVendorId(
      mapping[CanonicalField.VENDOR_ID]
        ? rawRow[mapping[CanonicalField.VENDOR_ID]]
        : null,
    );

    // NEW: Dimensional fields (all optional)
    const salesChannel = normalizeSalesChannel(
      mapping[CanonicalField.SALES_CHANNEL]
        ? rawRow[mapping[CanonicalField.SALES_CHANNEL]]
        : null,
    );
    ...
    const geography = normalizeGeography(
      mapping[CanonicalField.GEOGRAPHY_COUNTRY]
        ? rawRow[mapping[CanonicalField.GEOGRAPHY_COUNTRY]]
        : null,
      mapping[CanonicalField.GEOGRAPHY_REGION]
        ? rawRow[mapping[CanonicalField.GEOGRAPHY_REGION]]
        : null,
      mapping[CanonicalField.GEOGRAPHY_POSTAL_CODE]
        ? rawRow[mapping[CanonicalField.GEOGRAPHY_POSTAL_CODE]]
        : null,
    );

    const currency = normalizeCurrency(
      mapping[CanonicalField.CURRENCY]
        ? rawRow[mapping[CanonicalField.CURRENCY]]
        : null,
    );
  ...
  for (let i = 0; i < rawRows.length; i++) {
    ...
    if (normalized) {
      ...
      errors.push({
        ...
        error:
          "Failed to normalize row - missing required fields or invalid data",
      });
    }
  }
